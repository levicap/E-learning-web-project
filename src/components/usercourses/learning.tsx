...
function Learning() {
  ...
  const averageProgress = items.filter(item => item.type === "course").length > 0 
    ? items.filter(item => item.type === "course")
                ...
                {filteredItems.length === 0 ? (
                  ...
                  <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                    {currentItems.map((item) => (
                      <Card key={item._id} className="overflow-hidden group transition-all duration-300 hover:shadow-lg">
                            ...
                            <div className="space-y-2">
                              <div className="flex justify-between text-sm">
                                <span className="flex items-center gap-2">
                                  <BookOpen className="h-4 w-4" />
                                  Lessons Completed: {item.completedLessonsCount} / {item.lessons ? item.lessons.length : 0}
                                </span>
