...
// import { Camera, CameraOff, Mic, MicOff, UserMinus, UserPlus, MessageSquare, Users, X, Settings, ScreenShare, PhoneOff, Maximize, Minimize, Hand, Hand as HandOff, FileText } from 'lucide-react';
...
//     screenTrack: null
//   });
//   const [remoteUsers, setRemoteUsers] = useState<{ uid: string; videoTrack: ICameraVideoTrack; audioTrack: IMicrophoneAudioTrack; isScreenShare?: boolean }[]>([]);
...
//                 />
//                 <div className="absolute bottom-3 left-3 bg-background/80 backdrop-blur-sm px-3 py-1.5 rounded-md text-sm font-medium">
//                   Screen Share from User {user.uid.slice(0, 4)}
//                 </div>
//               </div>
//             ))}
...
//                 <ParticipantsList channel={channel} userId={userId} />
//               </TabsContent>
...
const VideoChat: React.FC<VideoChatProps> = ({ channel, onLeave, username, userId }) => {
  ...
  const [isFullScreen, setIsFullScreen] = useState(false);
  const [isHandRaised, setIsHandRaised] = useState(false);
  const [activeTab, setActiveTab] = useState<'chat' | 'participants' | 'whiteboard' | 'notes' | 'files' | null>(null);
  const [isRecording, setIsRecording] = useState(false);
  ...
  const toggleSidebar = (tab: 'chat' | 'participants' | 'whiteboard' | 'notes' | 'files') => {
    if (activeTab === tab) {
      setActiveTab(null);
    } else {
      setActiveTab(tab);
          ...
          <div className="w-80 border-l bg-card/50 backdrop-blur-sm">
            <Tabs value={activeTab} className="w-full">
              <TabsList className="w-full">
                ...
                {activeTab === 'files' && <TabsTrigger value="files">Files</TabsTrigger>}
              </TabsList>
              ...
              <TabsContent value="notes" className="h-[calc(100vh-12rem)]">
                <Notes roomId={channel} userId={userId} />
              </TabsContent>

              <TabsContent value="files" className="h-[calc(100vh-12rem)]">
                <FileSharing roomId={channel} userId={userId} />
              </TabsContent>
            </Tabs>
          </div>
        )}
          ...
          <div className="flex items-center gap-2 flex-wrap justify-center">
            ...
            <TooltipProvider>
              <Tooltip>
                ...
                <TooltipContent>
                  <p>{activeTab === 'files' ? "Hide files" : "Show files"}</p>
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
          </div>
